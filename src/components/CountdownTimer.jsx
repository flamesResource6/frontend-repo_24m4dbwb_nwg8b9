...
export default function CountdownTimer({ targetDate }) {
  ...
  useEffect(() => {
    ...
    return () => mql.removeEventListener?.('change', onRM)
  }, [])
  ...
  return (
    ...
    >
      <div className="text-center mb-6">
        <motion.h2
          className="text-xl sm:text-3xl md:text-4xl font-extrabold tracking-wide text-amber-200 drop-shadow-[0_2px_10px_rgba(255,200,120,0.45)]"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
      ...
      <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6 md:gap-8 text-center">
        <TimeUnit label="Days" value={diff.days} reduceMotion={reduceMotion} />
        <Divider />
        <TimeUnit label="Hours" value={diff.hours} reduceMotion={reduceMotion} />
        <Divider />
        <TimeUnit label="Minutes" value={diff.mins} reduceMotion={reduceMotion} />
        <Divider />
        <TimeUnit label="Seconds" value={diff.secs} reduceMotion={reduceMotion} />
      </div>
      ...
      <motion.div
        ...
        className="mx-auto mt-8 w-[220px] sm:w-[340px] md:w-[420px]"
      >
        <div className="relative">
          ...
          <motion.div
            className="relative z-10 rounded-md border-4 border-amber-900 bg-gradient-to-b from-amber-200/90 to-amber-300/80 px-4 sm:px-6 py-2 text-center font-black tracking-widest text-amber-900 shadow-[inset_0_10px_20px_rgba(255,255,255,0.25),0_10px_30px_rgba(0,0,0,0.35)]"
            initial={{ y: -10 }}
...
function TimeUnit({ label, value, reduceMotion }) {
  return (
    <div className={`${unitStyle}`}>
      <div className="relative">
        <motion.div
          ...
          animate={reduceMotion ? undefined : { textShadow: [
